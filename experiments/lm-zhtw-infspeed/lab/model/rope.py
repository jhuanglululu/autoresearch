...
def apply_rope(x: torch.Tensor, cos: torch.Tensor, sin: torch.Tensor) -> torch.Tensor:
    ...
    RoPE is ABSOLUTE: the rotation applied to token position p depends only on p.
    The caller passes the cos/sin rows for the *absolute* positions of `x`'s T
    tokens (rows [0:T] in a plain forward; rows [offset:offset+T] when decoding a
    cached suffix — see GPT._rope_at). Because a token's rotated key never changes
    once computed, RoPE-applied keys are safe to store in a KV cache verbatim.
    """
    d = x.shape[-1]
    x1, x2 = x[..., : d // 2], x[..., d // 2 :]
